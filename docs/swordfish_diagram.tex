...
\begin{tikzpicture}[->,>=stealth']
   ...
   ] (MODEL) 
 {
  \textbf{Poisson Point Process}\\
      $I(\Omega|\btheta)$: Intensity\\
      $N(\Omega)$: Statistical error\\
      $\Sigma(\Omega, \Omega')$: Systematic error\\
...
%  anchor=center
 ...
 {%
   \textbf{Optimal observation strategy}
 };
...
%  anchor=center
 ...
 {%
   ...
   $\mathcal{I}_{ij}(\btheta) \to \color{red}\left(s_i(\btheta), b_i(\btheta)\right)$
 };
...
\path (INFO) edge[bend right=0] node[rotate=33, anchor=south]{Additive components} (COUNTS);
