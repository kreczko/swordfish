...
\begin{tikzpicture}[->,>=stealth']
 ...
 {
  \textbf{Model}\\
      $\Phi(\Omega|\theta)$: Predicted counts\\
      $\Phi(\Omega)_{BG}$: Background\\
      $\Sigma(\Omega, \Omega')$: Covariance\\
      $\mathcal{E}(\Omega)$: Exposure
 };
...
%  text width=3cm, 	% max text width
 ...
 {%
   ...
   ${\color{red}g_{ij}(\theta)} = \mathcal{I}_{ij}(\theta)$
 };

 \node[state, below of=GEO, node distance = 2.5cm, align=center] (CONTOUR)
 {
...
%  anchor=center
 ...
 {%
   ...
   $\mathcal{I}(\theta)_{ij} = \int dt\int d\Omega \frac{d\mathcal{E}(\Omega)}{dt}\color{red}\mathcal{F}(\Omega|\theta)_{ij}$
 };
...
%  anchor=center
 ...
 {%
   ...
   $\mathcal{I}_{ij}(\theta) \to \color{red}\left(s_i(\theta), b_i(\theta)\right)$
 };
